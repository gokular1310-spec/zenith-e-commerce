...
const SubAdminOrderDetailPage = () => {
  ...
  if (!order) {
    return <div className="text-center text-xl text-gray-500">Order not found or access denied.</div>;
  }
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 text-gray-700">
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
        ...
        <Link to="/sub-admin/orders" className="text-sm text-primary-600 hover:underline">&larr; Back to Orders</Link>
      </div>
      ...
      {/* Main Details */}
      <div className="md:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6">
      ...
      <div className="mt-6">
        ...
        <p className="text-sm text-gray-500 mb-4">You can only see and manage items that you have listed.</p>
            ...
            <table className="w-full text-sm text-left">
                ...
                <tbody>
                    {order.items.map(item => (
                        <tr key={item.productId} className={`border-b border-gray-200 ${!userProductIdsInOrder.has(item.productId) && 'opacity-50'}`}>
                            <td className="px-6 py-4 font-medium text-gray-900">{item.productName} {!userProductIdsInOrder.has(item.productId) && '(Other Vendor)'}</td>
